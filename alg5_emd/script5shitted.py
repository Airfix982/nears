...
def write_wav_float_mono(path, samples, sample_rate):
...
def read_wav_float_mono(path):
    ...
    if data.dtype not in (np.float32, np.float64):
        raise ValueError(f"expected float wav, got {data.dtype}")
...
def pad_to_multiple(samples, multiple):
    ...
    padded = np.pad(samples, (0, pad_len), mode="constant", constant_values=0)
...
def get_residue_scale():
    return 2 ** 20

def float_residue_to_fixed_int(residue, scale):
    return np.rint(residue * scale).astype(np.int64)

def fixed_int_to_float_residue(values, scale):
    return values.astype(np.float64) / scale


def encrypt_residue(residue, ks_row, meta, mode="xor", residue_scale=2**20, fixed_bits=64):
    # bits = meta["bits_per_sample"]
    # r_int = float_residue_to_pcm_int(residue, meta)
    r_int = float_residue_to_fixed_int(residue, residue_scale)

    if mode == "xor":
        r_u = r_int.view(np.uint64)
        out_u = np.bitwise_xor(r_u, ks_row.astype(np.uint64))
        return out_u

    if mode == "modadd":
        out_u = (r_int.view(np.uint64) + ks_row.astype(np.uint64))# % (1 << fixed_bits)
        return out_u
...
def decrypt_residue(residue_enc, ks_row, meta, mode="xor", residue_scale=2**20, fixed_bits=64):
    if mode == "xor":
        out_u = np.bitwise_xor(residue_enc.astype(np.uint64), ks_row.astype(np.uint64))
        out_i = out_u.view(np.int64)
        return fixed_int_to_float_residue(out_i, residue_scale)

    if mode == "modadd":
        out_u = (residue_enc.astype(np.uint64) - ks_row.astype(np.uint64))# % (1 << fixed_bits)
        out_i = out_u.view(np.int64)
        return fixed_int_to_float_residue(out_i, residue_scale)
    # bits = meta["bits_per_sample"]
...
def encrypted_fixed_to_float(values_u64, residue_scale):
    values_i64 = values_u64.view(np.int64)
    return values_i64.astype(np.float64) / residue_scale
...
def encrypt_matrix_rows(mat, ks_mat, meta, itr, mode, emd_max_imfs, emd_max_siftings):
    ...
    for _ in range(itr):
        ...
        for i in range(out.shape[0]):
            ...
            residue_enc_float = encrypted_fixed_to_float(residue_enc, 2**20)
            sig_enc = reconstruct_from_imfs_and_residue(imfs, residue_enc_float)
            new_mat[i, :] = sig_enc
        out = new_mat
    return out


def float_cipher_residue_to_u64(residue_enc_float, residue_scale):
    values_i64 = np.rint(residue_enc_float * residue_scale).astype(np.int64)
    return values_i64.view(np.uint64)
...
def decrypt_matrix_rows(mat, ks_mat, meta, itr, mode, emd_max_imfs, emd_max_siftings):
    ...
    for _ in range(itr):
        ...
        for i in range(out.shape[0]):
            ...
            residue_enc_u64 = float_cipher_residue_to_u64(residue_enc, 2**20)
            residue = decrypt_residue(residue_enc_u64, ks_mat[i, :], meta, mode=mode)
...
def build_keystream_u64(x, y):
    ...
    k = np.bitwise_xor(xi, yi).astype(np.uint64)
    return k
...
def encrypt_chunk(chunk_samples, meta, row_len, itr, mode, emd_max_imfs, emd_max_siftings):
    ...
    return enc_flat.astype(np.float64), h, pad_len
...
def decrypt_chunk(enc_chunk_samples, h, meta, row_len, itr, mode, pad_len, emd_max_imfs, emd_max_siftings):
    ...
    perm = build_permutation(len(dec_flat), h)
    restored = apply_inverse_scrambling(dec_flat, perm)
...
def encrypt_signal(samples, meta, chunk_size, row_len, itr, mode, emd_max_imfs, emd_max_siftings):
    ...
    encrypted = np.concatenate(enc_chunks).astype(np.float64) if enc_chunks else np.array([], dtype=np.float64)
...
def decrypt_signal(samples, hashes, pad_lens, meta, chunk_size, row_len, itr, mode, emd_max_imfs, emd_max_siftings):
    ...
    decrypted = np.concatenate(dec_chunks).astype(np.float64) if dec_chunks else np.array([], dtype=np.int64)
    return decrypted
...
def test_5_run(org, enc, dec, errored_enc=None, org_format=None, new_format=None):
    ...
    write_wav_float_mono(enc, encrypted, meta["sample_rate"])
    ...
    enc_samples, enc_meta = read_wav_float_mono(enc)
    ...
    write_wav_mono(dec, clip_to_pcm_range(decrypted, enc_meta), enc_meta)
